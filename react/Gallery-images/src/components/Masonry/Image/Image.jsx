...
const Image = (img) => {
  ...
  const getImageModal = async (id) => {
    ...
    const res = await fetch(
      `https://api.unsplash.com/photos/${id}?client_id=e5y8jIGj9QYvcG2_nmJEz1Zr_8hbzjB6IylJnef5kHY`
    )
    ...
    <>
      ...
      >
        {loading ? (
          ...
          <img src={img.img.urls?.small} alt='None' />
        )}
      </div>

      {/*Modal que tiene el color negro con opacidad  */}
      <div
        className={modal ? styles.modal_contain : styles.modal_containOff}
        onClick={() => handleClick(false)}
      ></div>

      {/* Card que contiene lla foto seleccionada */}
      <div className={modal ? styles.modal : styles.disable}>
        <button className={styles.close} onClick={() => handleClick(false)}>
          X
        </button>
        {loading ? (
          <img src={imgLoading} width={54} />
        ) : (
          <img
            src={imgModal[0]?.urls.small}
            alt=''
